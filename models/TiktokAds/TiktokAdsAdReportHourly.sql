{% if var('TiktokAdsAdReportHourly') %}
{{ config( enabled = True ) }}
{% else %}
...
{{set_table_name('%tiktokads%ad_report_hourly')}}    
{% endset %}
...
{% for i in results_list %}
    ...
    FROM (
        ...
        DENSE_RANK() OVER (PARTITION BY Time, AdID order by {{daton_batch_runtime()}} desc) row_num
        FROM  {{i}} a
                {% if is_incremental() %}
