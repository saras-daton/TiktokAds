{% if var('TiktokAdsReservationCampaignReportDaily') %}
{{ config( enabled = True ) }}
{% else %}
...
{{set_table_name('%tiktokads%reservation_campaign_report_daily')}}    
{% endset %}
...
{% for i in results_list %}
    ...
    FROM (
        ...
        DENSE_RANK() OVER (PARTITION BY Date, CampaignID order by {{daton_batch_runtime()}} desc) row_num
        FROM  {{i}} a
                {% if is_incremental() %}
